...
class UnityNodeTypeResolver : INodeTypeResolver
{
    ...
    public bool Resolve(NodeEvent nodeEvent, ref Type currentType)
    {
        ...
        {
            if (!int.TryParse(nodeEvent.Tag.Value.Replace(UnityTagPrefix, ""), out var unityObjectId))
            {
                return false;
            }
...
{
    public readonly static Dictionary<int, string> IdToTypeName = new Dictionary<int, string>()
    {
        { 1,  "GameObject" },
        { 2,  "Component" },
        { 3,  "LevelGameManager" },
    ...
    public static Dictionary<string, int> type_name_To_ID = TypeNameToIDConstructor();

    private static Dictionary<string, int> TypeNameToIDConstructor()
    {
        var typeNameToID = new Dictionary<string, int>();
        foreach(KeyValuePair<int,string> pair in IdToTypeName)
        {
            typeNameToID.Add(pair.Value, pair.Key);
        }
